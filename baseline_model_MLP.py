...
try:
    ...
    from skorch.callbacks import EarlyStopping, EpochScoring, Callback
    HAVE_TORCH = True
except Exception:
    HAVE_TORCH = False
...
def mlp_space_gpu():
    if HAVE_BAYES:
        return {
            "model__module__hidden_dim": Categorical([128, 256]),
            "model__module__dropout": Real(0.1, 0.3),          # narrower, safer
            "model__lr": Real(5e-4, 5e-3, prior="log-uniform"),
            "model__batch_size": Categorical([128, 256, 512]),
        }
    else:
        return {
            "model__module__hidden_dim": [128, 256],
            "model__module__dropout": [0.1, 0.2, 0.3],
            "model__lr": [5e-4, 1e-3, 2e-3, 5e-3],
...
if HAVE_TORCH:
    ...
    class SetInputDim(Callback):
        """Set module__input_dim from the transformed X just once before training."""
        def on_train_begin(self, net, X, y):
            if hasattr(X, 'shape'):
                n_features = X.shape[1]
                # This triggers a (single) re-init with the correct input_dim
